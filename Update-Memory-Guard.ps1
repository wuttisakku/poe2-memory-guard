...
try {
    ...
    $nestedRoot = Get-ChildItem -LiteralPath $extractPath -Directory | Where-Object {
        (Test-Path -LiteralPath (Join-Path $_.FullName 'PoE2MemoryGuard.cmd')) -or
        (Test-Path -LiteralPath (Join-Path $_.FullName 'Start-Memory-Guard.cmd'))
    } | Select-Object -First 1
    if ($nestedRoot) {
