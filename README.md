...
- On launch, the guard asks for Administrator permission when needed, checks the latest GitHub release, and updates itself when a newer ZIP is available.
- The guard can be started before the game and will wait until a matching process appears.

### Quick start

1. Double-click `Start-Memory-Guard.cmd` and select **Yes** if Windows asks for Administrator permission.
